const queries = [
 ...
 Invoice_Contractor.manager_price/1000000 as manager_price,
 ...
 From  contract as con left join town on town.id =con.town_id
  ...
  (select g2j(end_date+ 1) from extension  where contract_Id=Ag.id order by no_id desc limit 1) as extension_end_date ,
  g2j(Ag.record_letter_date+ 1) as record_letter_date,  
 (select  b2.title from invoice_consultant LEFT JOIN baseinfo b2 ON invoice_consultant.no_id= b2.id  
  ...
  g2j(inv.start_date+ 1) as period_start_date,
  g2j(inv.end_date+ 1) as period_end_date,
  g2j(inv.letter_date_branch+ 1)as letter_date_branch,
  g2j(inv.letter_date_manager+ 1) as letter_date_manager,
  g2j(inv_approve.letter_date_employer+ 1) as letter_date_employer,
  g2j(inv_Pay.pay_date+ 1) AS pay_date,
...
{ key: 'Web_insurance', query:`select town.title as town ,con.title,con.contract_no,co.title as company,
     ...
     (coalesce((select g2j(end_date+ 1) from Extension where type_id=2 and contract_id=con.id order by end_date desc limit 1) , g2j(con.end_date+ 1))) as date_end_extension ,
     ins.fund/1000000 as fund,
     ...
     (coalesce((select g2j(end_date+ 1) from insurance_Appendix where contract_id=con.id order by end_date desc limit 1) , g2j(ins.end_date+ 1))) as date_expire,
 ...
 { key: 'Web_contractCycle', query:`select town.title as town, con.title,con.contract_no,co.title as company, b3.title as status_title,con.duration,
                ...
                DATE_PART('day',(select commision_date from temp_Delivery  where contract_Id=con.id order by temp_Delivery.commision_date  desc limit 1)- (coalesce((select (end_date) from extension  where contract_Id=con.id order by no_id  desc limit 1),con.end_date) ::timestamp))as Duration_Time,
              ...
              DATE_PART('day',NOW()- (select  (commision_date+ 1)  from temp_Delivery  where contract_Id=con.id order by temp_Delivery.commision_date  desc limit 1)) as commision_end_date,
...
{ key: 'Web_tender', query:`select tn.title,b8.title as province,town.title as town,b3.title as service_type,tender_no,
                   ...
                   g2j(tn.winner_letter_date+ 1) as winner_letter_date,
                   tn.winner_letter_no,
                   g2j(contract.contract_date+ 1) as contract_date,
                   ...
                   contract.announcement_date ::date - winner_letter_date::date as winner_to_signification
 ...
 { key: 'Web_contract', query:`  select  town.title as town,con.title, con.contract_no,
                  ...
                  (coalesce((select g2j(end_date+1) from Extension where   contract_Id=con.id order by no_id  desc limit 1) , g2j(con.end_date+1)))  as  ex_end_date ,
                  ...
                  (coalesce((select contract_new_price from value_Change  where contract_Id=con.id order by no_id desc limit 1) ,  con.initial_amount))::numeric, 2)as  pishraft_mali,
                  ...
                  (coalesce((select g2j(end_date+1) from Extension where type_id=2 and contract_id=Ag.id order by end_date desc limit 1) , g2j(Ag.end_date+1))) as  agex_end_date ,
                   co2.title as company_study_agreement,
                  ag_2.contract_no  as contract_no_study_agreement,
                  g2j(ag_2.contract_date+1) as contract_date_study_agreement
...
{ key: 'Web_Document', query:`select 
         ...
         con.file_delivery as file_delivery,
      ...
      (select file_late from extension   where contract_Id=con.id order by extension.no_id desc limit 1) as file_late,
      (select file_plan_pdf from extension   where contract_Id=con.id order by extension.no_id desc limit 1) as file_plan_pdf,
      (select file_plan_msp from extension   where contract_Id=con.id order by extension.no_id desc limit 1) as file_plan_msp,
       ...
       (select file_contract from insurance   where contract_Id=con.id order by insurance.end_date desc limit 1) as  in_file_contract,
         ...
         (select file_record from temp_Delivery   where contract_Id=con.id order by temp_Delivery.commision_date desc limit 1) as  td_file_record,
      ...
      (select file_invoice from invoice_Contractor   where contract_Id=con.id order by invoice_Contractor.no_id desc limit 1) as inv_file_invoice,
